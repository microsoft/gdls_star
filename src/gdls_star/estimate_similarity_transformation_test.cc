...
class EstimateSimilarityTransformationTest : public ::testing::Test {
 public:
  ...
  bool EvaluateSolution(const TestParameters& test_params,
                        const SimilarityTransformation& expected_transformation,
                        const GdlsStar::Solution& estimated_transformations) {
  ...
  void ExecuteRandomTest(
    ...
    const GdlsStar::Solution solution =
        EstimateSimilarityTransformation(priors, correspondences);
...
TEST_F(EstimateSimilarityTransformationTest,
       MinimalBasicEstimationNoPriorsNoNoise) {
  TestParameters test_params;
  const GdlsStar::Priors priors;
  for (int i = 0; i < kNumTrials; ++i) {
    const SimilarityTransformation expected_transformation =
        GenerateRandomSimilarityTransformation(test_params);
    ExecuteRandomTest(test_params, expected_transformation, priors);
  }
}

TEST_F(EstimateSimilarityTransformationTest,
       MinimalBasicEstimationWithNoiseNoPriors) {
  TestParameters test_params;
...
TEST_F(EstimateSimilarityTransformationTest,
       NonMinimalBasicEstimationNoPriorsNoNoise) {
  TestParameters test_params;
...
TEST_F(EstimateSimilarityTransformationTest,
       NonMinimalBasicEstimationWithNoiseNoPriors) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
...
// Minimal estimatio tests w/ priors.
TEST_F(EstimateSimilarityTransformationTest,
       MinimalBasicEstimationWithPriorsNoNoise) {
  const TestParameters test_params;
  GdlsStar::Priors priors;
  priors.scale_penalty = 1.0;
  priors.gravity_penalty = 1.0;
  for (int i = 0; i < kNumTrials; ++i) {
    const SimilarityTransformation expected_transformation =
        GenerateRandomSimilarityTransformation(test_params);
    priors.scale_prior = expected_transformation.scale;
    priors.world_down_direction = Eigen::Vector3d::UnitZ();
    priors.query_down_direction =
        expected_transformation.rotation * priors.world_down_direction;    
    ExecuteRandomTest(test_params, expected_transformation, priors);
  }
}

TEST_F(EstimateSimilarityTransformationTest,
       MinimalBasicEstimationWithPriorsAndNoise) {
  TestParameters test_params;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
  test_params.translation_thresh = 1.0;
...
// Non-minimal estimatio tests w/ priors.
TEST_F(EstimateSimilarityTransformationTest,
       NonMinimalBasicEstimationWithPriorsNoNoise) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  GdlsStar::Priors priors;
  priors.scale_penalty = 1.0;
...
TEST_F(EstimateSimilarityTransformationTest,
       NonMinimalBasicEstimationWithPriorsAndNoise) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
...
TEST_F(EstimateSimilarityTransformationTest, AllInliersWithNoiseNoPriors) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
  ...
  for (int i = 0; i < kNumTrials; ++i) {
    const SimilarityTransformation expected_transformation =
        GenerateRandomSimilarityTransformation(test_params);
    ExecuteRansacRandomTest(test_params, expected_transformation, priors);
  } 
}

// RANSAC w/ inliers and outliers and noise but w/o priors.
TEST_F(EstimateSimilarityTransformationTest,
       InliersAndOutliersWithNoiseNoPriors) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
...
// RANSAC w/ inliers and outliers, noise, and priors.
TEST_F(EstimateSimilarityTransformationTest,
       InliersAndOutliersWithNoiseAndPriors) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
...
// RANSAC w/ inliers and outliers, noise, and gravity prior.
TEST_F(EstimateSimilarityTransformationTest,
       InliersAndOutliersWithNoiseAndGravityPrior) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
  ...
  test_params.inlier_ratio = 0.8;
  GdlsStar::Priors priors;
  priors.scale_penalty = 0.0;  // Disabling scale prior.
  priors.gravity_penalty = 1.0;
  for (int i = 0; i < kNumTrials; ++i) {
    const SimilarityTransformation expected_transformation =
        GenerateRandomSimilarityTransformation(test_params);
...
// RANSAC w/ inliers and outliers, noise, and scale prior.
TEST_F(EstimateSimilarityTransformationTest,
       InliersAndOutliersWithNoiseAndScalePrior) {
  TestParameters test_params;
  test_params.num_points = kNumPoints;
  test_params.noise_std_dev = 1e-3;
  test_params.rotation_thresh = DegToRad(5.0);
  ...
  GdlsStar::Priors priors;
  priors.scale_penalty = 1.0;
  priors.gravity_penalty = 0.0;  // Disabling gravity prior.
  for (int i = 0; i < kNumTrials; ++i) {
    const SimilarityTransformation expected_transformation =
        GenerateRandomSimilarityTransformation(test_params);
    priors.scale_prior = expected_transformation.scale;
